...
Game::Game()
    ...
    , settingsPage(0), antiAlias(true), perfMode(false)
...
void Game::shutdown() {
    ...
    // 清理子弹池
    for (auto* b : playerBulletPool) delete b;
    for (auto* b : enemyBulletPool) delete b;
...
void Game::handleInput() {
    ...
    if (state == GameState::MENU && menuCooldown <= 0) {
        ...
        if (enter) {
            ...
            case MenuOption::SETTINGS_MENU: settingsPage=0; settingsSelection=currentResolution; state=GameState::SETTINGS; break;
    ...
    if (state == GameState::SETTINGS && menuCooldown <= 0) {
        ...
        // 翻页
        if (left)  { settingsPage=(settingsPage-1+3)%3; settingsSelection=0; menuCooldown=10; }
        if (right) { settingsPage=(settingsPage+1)%3;   settingsSelection=0; menuCooldown=10; }

        // 当前页操作
        if (settingsPage == 0) {  // 分辨率
...
void Game::renderSettings(Renderer& r) {
    ...
    if (settingsPage == 0) {
        ...
        for (int i = 0; i < RESOLUTION_COUNT; i++) {
            bool sel = (i == settingsSelection), act = (i == currentResolution);
        ...
        for (int i = 0; i < 2; i++) {
            ...
            wchar_t buf[80];
            swprintf_s(buf, _countof(buf), (i==settingsSelection) ? L"> %s:   [%s]  %s" : L"   %s:    %s  [%s]",
                       optNames[i], on ? L"ON " : L"   ", on ? L"   " : L"OFF");
            r.drawTextLeft(buf, 50*sc, contentY+i*optStep, WINDOW_WIDTH-50*sc, optStep, ff, 16*sc, FontStyleRegular, col);
        }
        if (d2dAvailable) {
            ...
            wchar_t buf[80];
            swprintf_s(buf, _countof(buf), (i==settingsSelection) ? L"> %s:   [%s]  %s" : L"   %s:    %s  [%s]",
                       optNames[i], useDirect2D ? L"D2D" : L"    ", useDirect2D ? L"    " : L"GDI+");
            r.drawTextLeft(buf, 50*sc, contentY+i*optStep, WINDOW_WIDTH-50*sc, optStep, ff, 16*sc, FontStyleRegular, col);
        }
    }
    ...
    r.drawTextCentered(settingsPage == 2 ?
        L"Arrows=Select  Enter=Toggle  Left/Right=Page  ESC=Back" :
        L"Arrows=Select  Enter=Apply  Left/Right=Page  ESC=Back",
...
void Game::renderGameOver(Renderer& r) {
    ...
    float sc=(std::min)(WINDOW_HEIGHT/480.0f, 2.2f); bool vic=(state==GameState::VICTORY);
    const wchar_t* ff=L"Arial";
