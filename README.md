# Plane War — Direct2D Edition

经典纵版飞行射击游戏，基于 Win32 + GDI+ / Direct2D 双渲染后端实现。
...
- **5 个关卡**：普通敌机 → 快速敌机 → 射击敌机 → Boss 战，难度递增
- **双渲染后端**：Direct2D（硬件加速）/ GDI+（兼容回退），运行时一键切换
- **自适应分辨率**：支持 4:3 / 16:9 / 16:10 / 21:9 / 32:9 共 20 种分辨率
- **高 DPI 支持**：渲染目标强制 96 DPI，任意缩放比例下 UI 一致
- **可变游戏速度**：慢速 45 / 中速 60 / 快速 90 tick/s 三档可调
- **5 种皮肤**：Classic / Neon / Lava / Aurora / Mono
- **帧率上限**：60 / 75 / 120 / 144 / 165 / 240 Hz，游戏逻辑与画面刷新率解耦
- **性能监视器**：实时 FPS + 内存占用显示
- **对象池**：子弹复用，避免频繁分配
...
- **护盾修复**：子弹击中护盾时蓝色格挡特效，护盾立即消失，不扣血不震动
- **HUD 字体增大**：主字体 12→18px，行高同步调整
